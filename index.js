...
function createDictionary(entries) {
	...
	entries.forEach((value, key, map) => { dictionary.set(value, key, map) })
	return new Map(dictionary.keys());
}
...
function getKeysStartingWith(dictionary, letter) {
	...
	// Повертаємо масив
	let dictionaryArr = new Array();
	for (const [key] of dictionary) {
		if (key.startsWith(letter)) {
			dictionaryArr.push(key);
		}
	}
...
function checkPresence(dictionary, keys) {
	...
	// Повертаємо масив
	let presenceArr = new Array();
	keys.forEach((key) => {
		presenceArr.push(dictionary.has(key));
	})
	return presenceArr
}
...
function sortByValues(dictionary) {
	...
	let sortedDictionary = new Map();

	let sortedArray = Array.from(dictionary.entries()).sort((a, b) => b[1] - a[1]);
 ...
 * Значенням множини є ключі словника, де рядкові ключі переводяться в верхній регістр, а числові ключі збільшуються на 1.
 * dictionary - Словник, який потрібно перетворити.
